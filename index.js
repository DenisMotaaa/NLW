...
const fazerCheckIn = (event) =>{
  //confirmar checkIn?
  const mensagem =  "tem certeza que deseja fazer checkIn?";
  if(confirm(mensagem) == false){
    return alert("cancelado com sucesso")
  }else alert("checkIn confirmado")
  //encontrar participante da lista
  const participante = participantes.find((p) =>{
    return p.email == event.target.dataset.email
  })
  // att checkIn do participante
  participante.dataCheckIn = new Date()
  // att lista de participantes
  atualizarLista(participantes)
